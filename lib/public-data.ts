...
export async function publicData():Promise<PublicData>{const [repos,job,state]=await Promise.all([savedRepos(),readJob(),readSyncState()]);const fallback=state?.phase==='completed'?state.updatedAt:Math.max(0,...repos.map(r=>r.historyFetchedAt??r.fetchedAt))||null;return {repos:repos.map(publicRepo),lastUpdated:job?.lastSuccessfulAt??fallback,updating:job?.status==='running'&&Date.now()-job.updatedAt<300000,incomplete:repos.some(r=>!currentHistory(r)||r.daily.length<8)};}
