...
`SYNC_SCHEDULE_UTC` is the intended daily UTC time (default 09:00); changing it does not register or reschedule a remote service. This source tree includes a disabled GitHub Actions workflow at `.github/workflows/gitpulse-sync.yml`; enable it only after configuring `GITPULSE_SCHEDULER_ENABLED`, `GITPULSE_ORIGIN`, and the server-side `SYNC_CRON_SECRET`. The runner follows continuation IDs, stops on rate limits and prints only safe result summaries. Vercel Cron or another trusted scheduler can call the same protected endpoint. Next automatic run remains unset until an external scheduler is enabled.
